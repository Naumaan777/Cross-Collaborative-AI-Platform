...
export async function runDestinationAgent(state: TripState): Promise<AgentTrace> {
    const trace: AgentTrace = { agentName: 'Destination Agent' as const, executed: true };
    ...
    try {
        const response = await groq.chat.completions.create({
            ...
            messages: [
                ...
                { role: 'user', content: `Analyze this user request and suggest matching destinations: "${state.userQuery}"` }
            ],
            temperature: 0.3
        });

        trace.output = response.choices[0]?.message?.content || 'No destinations generated.';
        trace.decisionJustification = "Evaluated climate, regional preferences, and explicit user-defined constraints.";
    } catch (error) {
        // Destructure securely with type guard predicates
...
export async function runItineraryAgent(state: TripState, destinationOutput: string): Promise<AgentTrace> {
    const trace: AgentTrace = { agentName: 'Itinerary Agent' as const, executed: true };
  ...
  `;
    ...
    try {
        const response = await groq.chat.completions.create({
            ...
            messages: [
                { role: 'system', content: systemInstruction },
                { role: 'user', content: `User Context: "${state.userQuery}". Based on these suggested options:\n${destinationOutput}\nCreate a structured, logical day-by-day draft itinerary.` }
            ],
            temperature: 0.5
        });

        trace.output = response.choices[0]?.message?.content || 'No itinerary created.';
    } catch (error) {
        const exception = error && typeof error === 'object' ? error : {};
...
export async function runBudgetAgent(state: TripState, itineraryOutput: string): Promise<AgentTrace> {
    const trace: AgentTrace = { agentName: 'Budget Agent' as const, executed: true };
  ...
  `;
    ...
    try {
        const response = await groq.chat.completions.create({
            ...
            messages: [
                { role: 'system', content: systemInstruction },
                { role: 'user', content: `User Query Context: "${state.userQuery}". Evaluate the total estimated costs for this proposed timeline and verify it strictly satisfies financial boundaries:\n${itineraryOutput}` }
            ],
            temperature: 0.2
        });

        trace.output = response.choices[0]?.message?.content || 'No budget breakdown provided.';
    } catch (error) {
        const exception = error && typeof error === 'object' ? error : {};
